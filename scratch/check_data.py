...
def make_labels(df, task):
    ...
    if "Inflection Label" in df.columns:
        infl_lookup = {
            x: i for i, x in enumerate(sorted(set(df["Inflection Label"])))
        }
        infl_labels = np.array(
            [infl_lookup[x] for x in df["Inflection Label"]], dtype=int
        )
    else:
...
def main():
    for ds in DATASETS:
        ...
        for task in TASKS:
            ...
            pat = os.path.join(
                OUTPUT_DIR, "**", f"*{ds}_reps", "sampled_indices.csv"
            )
