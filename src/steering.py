...
def load_probes(probe_dir, n_layers, probe_type, input_dim):
    """Loads all trained probes for each layer."""
    probes = {}
    device = get_device()
    
    # Load label map, which is crucial for mapping predictions back to inflections
    ...
    for layer in range(n_layers):
        if probe_type in ["mlp", "nn"]:
            ...
            if not os.path.exists(probe_path):
                utils.log_info(f"Warning: Probe for layer {layer} not found at {probe_path}, skipping.")
                continue
            
            # Initialize a new probe model and load its saved state
...
def compute_aivs(activations, labels, min_samples=10):
    """
    Computes Average Inflection Vectors (AIVs).
    An AIV is the mean activation for all examples of a specific inflection.
    """
...
def run_steering(args):
    """Main function to run the steering experiment."""
    utils.log_info("Starting steering experiment...")
    
    os.makedirs(args.output_dir, exist_ok=True)

    # Load activations and determine model dimensions
    ...
    # Load the pre-trained probes for each layer
    probes, label_map = load_probes(args.probe_dir, n_layers, args.probe_type, hidden_dim)
    if not probes:
        utils.log_info("No probes found in the specified directory. Exiting.")
        return

    # Prepare labels, filtering for those present in the probe's label map
    ...
    # Select a subset of words to act as targets for steering
    ...
    for layer in tqdm(sorted(probes.keys()), desc="Steering Layers"):
        ...
        # Load activations for the current layer and filter them
        activations = load_layer(shards, layer)
        activations = activations[valid_mask]

        # Compute AIVs for this layer
        aivs = compute_aivs(activations, inflection_labels_int)
        if not aivs:
            utils.log_info(f"Not enough samples to compute AIVs for layer {layer}. Skipping.")
            continue
        ...
        for i in range(len(target_df)):
            ...
            # Get original prediction probabilities from the probe
            ...
            # Steer towards every other inflection
            for i_steer in aivs.keys():
                if i_steer == i_orig:
                    continue
                
                # The steering vector is the direction from the original inflection's AIV to the target's.
                steering_vec = aivs[i_steer] - aivs[i_orig]
                h_steered = h_orig + args.lambda_steer * steering_vec

                # Get new probabilities from the steered activation
                if args.probe_type in ["mlp", "nn"]:
                    with torch.no_grad():
                ...
                # Calculate the two metrics of interest
    ...
    # Create and save a summary aggregated by layer
...
def parse_args():
    ...
    parser.add_argument("--probe_type", choices=["mlp", "nn", "rf"], default="mlp", help="Type of probe used for the experiment.")
    parser.add_argument("--lambda_steer", type=float, default=1.0, help="Lambda coefficient for steering vector strength.")
    parser.add_argument("--num_samples", type=int, default=100, help="Number of target words to use for steering.")
