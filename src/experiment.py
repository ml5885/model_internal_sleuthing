...
def main():
    ...
    parser.add_argument("--model", type=str, default="gpt2",
                        help="Model key to use (e.g. 'gpt2').")
    parser.add_argument("--dataset", type=str, required=True,
                        help="Dataset label (e.g., ud_gum_dataset).")
    parser.add_argument("--experiment", type=str,
                        choices=["multiclass_inflection", "lexeme"],
                        help="Run only the specified experiment (if omitted, both experiments are run).")
    ...
    # Define both experiment configurations (no longer passing control flags here)
    experiments = [
        {
            "name": "multiclass_inflection",
            "args": [
    ...
    # If the user specified one experiment, filter the list
    if args.experiment:
    ...
    # Finally, run the unsupervised analysis
