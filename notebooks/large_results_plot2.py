...
def plot_combined_probes(
    ...
    for row, (task, probe) in enumerate(combos):
        ...
        # Row labels
        if row == 0:
            axes[row, 0].set_ylabel("Linear", fontsize=18, labelpad=15)
        elif row == 1:
            axes[row, 0].set_ylabel("MLP", fontsize=18, labelpad=15)
        elif row == 2:
            axes[row, 0].set_ylabel("Linear", fontsize=18, labelpad=15)
        elif row == 3:
            axes[row, 0].set_ylabel("MLP", fontsize=18, labelpad=15)
        for col in range(n_cols):
            if row == n_rows - 1:
                axes[row, col].set_xlabel("Layer (% of model)", fontsize=18)
            if col == 2:
                axes[row, col].set_ylim(-1, 1)
                axes[row, col].set_ylabel("Selectivity", fontsize=18, labelpad=5)
                axes[row, col].set_yticks([-1, -0.5, 0, 0.5, 1])
            else:
                axes[row, col].set_ylim(0, 1)
                if col == 0:
                    axes[row, col].set_ylabel(f"{'Linear' if row % 2 == 0 else 'MLP'} Accuracy", fontsize=18, labelpad=15)
                else:
                    axes[row, col].set_yticklabels([])
                    axes[row, col].set_ylabel("")
            axes[row, col].tick_params(axis="x", rotation=45, labelsize=16, width=2, length=10)
            axes[row, col].tick_params(axis="y", labelsize=16, width=2, length=10)
